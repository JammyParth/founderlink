...
export class MessagesComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  selectPartner(partner: ConversationPartner): void {
    ...
    this.pollInterval = setInterval(() => this.pollMessages(partner.userId), 2000);
  }
  ...
  /** Silent background poll — only appends genuinely new messages */
  private pollMessages(partnerId: number): void {
    this.messagingService.getConversation(partnerId).subscribe({
      next: env => {
        const fresh = env.data ?? [];
  ...
  formatDate(date: string): string {
    const normalized = date.endsWith('Z') || date.includes('+') ? date : date + 'Z';
    return new Date(normalized).toLocaleDateString('en-IN', { year: 'numeric', month: 'short', day: 'numeric' });
  }
}

...
export class NotificationsComponent implements OnInit, OnDestroy {
  ...
  private startupNames = new Map<number, string>();
  ...
  ngOnInit(): void {
    this.startupService.getAll().subscribe({
      next: env => {
        (env.data ?? []).forEach(s => this.startupNames.set(s.id, s.name));
      }
    });
  ...
  formatMessage(message: string): string {
    return message.replace(/startup #(\d+)/gi, (_, id) => {
      const name = this.startupNames.get(Number(id));
      return name ? `startup "${name}"` : `startup #${id}`;
    });
  }

  formatDate(date: string): string {
    // Append 'Z' if no timezone info — backend sends LocalDateTime without offset

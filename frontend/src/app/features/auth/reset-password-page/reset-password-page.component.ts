  ...
  template: `
    <div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div class="max-w-md w-full space-y-8 bg-white p-8 rounded-lg shadow-md">
        <div>
          <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900">Enter Reset PIN</h2>
        </div>
          ...
          <div class="rounded-md shadow-sm space-y-4">
            <div>
              ...
              <input id="email" type="email" formControlName="email" 
                     class="mt-1 appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm bg-gray-50" 
                     readonly>
            </div>

            <div>
              <label for="pin" class="block text-sm font-medium text-gray-700">6-Digit PIN</label>
              <input id="pin" type="text" formControlName="pin" maxlength="6"
        ...
        <div class="text-center mt-4">
          <p class="text-sm text-gray-600">
            <a routerLink="/auth/login" class="font-medium text-blue-600 hover:text-blue-500">Back to sign in</a>
          </p>
        </div>
      </div>
    </div>
  ...
  onSubmit() {
    ...
    this.authService.resetPassword(this.resetForm.getRawValue()).subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        this.isLoading = false;
        this.error = err.error?.message || 'Invalid PIN or error resetting password';
      }
    });
  }
}
